...
deploy_logger = logging.getLogger('partnr.uploader')
...
def upload_arg_parser():
    parser = argparse.ArgumentParser(
        description='Upload partnr frontend static files.',
    )
...
def upload_files(bucket):
    ...
    for root, subdirs, files in os.walk(Constants.DEST_DIR):
        for fname in files:
            reldir = os.path.relpath(root, Constants.DEST_DIR)
            relfile = os.path.join(reldir, fname)
            # boto3 doesn't like relative files
            if relfile.startswith('./'):
                uploadfile = relfile[2:]
                relfile = relfile[2:]
            else:
                uploadfile = relfile
            # get the file extension
            _, file_extension = os.path.splitext(relfile)
            deploy_logger.info('Uploading {local_file} to {remote_file} in the {bucket} bucket'.format(
                local_file=relfile, remote_file=uploadfile, bucket=bucket.name))
            bucket.upload_file(relfile, uploadfile,
                ExtraArgs={'ContentType': get_content_type_from_ext(file_extension)})
...
def upload(override_args=None):
    ...
    deploy_logger.info('cd-ing to dest directory')
    os.chdir(Constants.DEST_DIR)
